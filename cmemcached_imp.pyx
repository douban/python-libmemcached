...
cdef class Client:
    ...
    def _preprocess_key(self, key):
        return self._normalize_to_bytes(self._use_prefix(key))
    ...
    def _normalize_to_bytes(self, key, encoding="utf-8"):
    ...
    def delete_multi(self, keys, time_t time=0, return_failure=False):
        ...
        for key in keys:
            ...
            if key_len >= MEMCACHED_MAX_KEY:
                continue
            retval = memcached_delete(self.mc, c_key, key_len, time)
            if retval not in (MEMCACHED_SUCCESS, MEMCACHED_NOTFOUND):
                failed_keys.append(key)
        #self.set_behavior(BEHAVIOR_NOREPLY, 0)
